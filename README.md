...
Successful responses contain the final text returned by the agent. For the example
prompt above, a possible response is shown below. The exact wording depends on the
Ollama model and may differ between requests:

```json
"Generative AI uses machine-learning models to create new text, images, audio, or code. It learns patterns from large datasets and generates outputs based on a user's instructions. Unlike traditional software, which follows explicitly programmed rules, generative AI produces novel content by predicting what should come next."
```

Because the FastAPI route returns the agent's text directly, the response is
serialized as a JSON string. If a prompt asks the agent to return JSON, the JSON
generated by the model is therefore contained inside that response string.
